...
\def\forkindep{\mathrel{\raise0.2ex\hbox{\ooalign{%
  \rustexBREAK\hidewidth$\rustexBREAK\vert\rustexBREAK$\hidewidth\rustexBREAK\cr\raise-0.9ex\hbox{$\smile$}}}}} 

\def\forkindepb{\mathrel{\hbox{\ooalign{%
\hidewidth$\vert$\hidewidth\cr\hbox{$\smile$}}}}}

%\def\ooalign#1{\begingroup
%  \newdimen\ooaligndimenI
%  \ooaligndimenI=0pt
%  \setbox0\hbox\bgroup
%    \def\cr{\egroup\setbox1\lastbox
%      \ifdim\wd1>\ooaligndimenI
%        \global\ooaligndimenI\wd1\relax
%      \fi
%      \hbox to0pt{\unhbox1}\hbox\bgroup
%    }
%    \let\crcr\cr
%    \hbox\bgroup
%      #1
%    \egroup\setbox1\lastbox
%    \ifdim\wd1>\ooaligndimenI
%      \global\ooaligndimenI\wd1\relax
%    \fi
%    \hbox to0pt{\unhbox1}%
%  \egroup
%  \hbox to\ooaligndimenI{\hss\unhbox0\hss}%
%\endgroup}
...
\begin{tabular}{l|p{4.6cm}|p{4.6cm}}
  &\textbf{Humanly} & \textbf{Rational}\\\hline
  \textbf{Thinking} & 
                ``\emph{The exciting new effort to make computers think \ldots
                      machines with human-like minds}''\hfill\cite{Haugeland:aitvi85}&
             ``\emph{The formalization of mental faculties in terms of
                                                    computational models}''\hfill\cite{ChaMcD:itai85}
  \\\hline
  \textbf{Acting} & 
              ``\emph{The art of creating machines that perform actions requiring
              intelligence when performed by people}''\hfill\cite{Kurzweil:taoim90} &
            ``\emph{The branch of CS concerned with the
            automation of appropriate behavior in complex situations}''\hfill\cite{LugStu:aisscps93}
  \\\hline
\end{tabular}
