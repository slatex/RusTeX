...
impl Interpreter<'_> {
    ...
    pub fn change_command(&mut self,cmdname:TeXStr,proc:Option<TeXCommand>,globally:bool) {
        /*if cmdname.to_string() == "pgfsysprotocol@currentprotocol" ||
            cmdname.to_string() == "ifpgfsysprotocol@buffered" ||
            cmdname.to_string() == "pgf@arrow@code" ||
            cmdname.to_string() == "pgf@ar@cache@1" {//(cmdname == TeXStr::new(&[0,1,2,3,4,255,254,253,252,251,95])) {//"scr@dte@chapter@init" || cmdname.to_string() == "scr@dte@tocline@init") {
